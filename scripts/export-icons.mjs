...
for (const size of sizes) {
  ...
  try {
    await run(executable, ['--width', String(size), '--height', String(size), '--output', output, source]);
  } catch (error) {
    if (error.code === 'ENOENT') {
      throw new Error('找不到 rsvg-convert。请安装 librsvg，或通过 RSVG_CONVERT 指定可执行文件。', { cause: error });
    }
    throw error;
  }

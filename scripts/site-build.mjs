...
import { content, scenes, mainScenes, storeImages } from '../website/content.mjs';
...
const assetManifest = JSON.parse(await readFile('store/manifest.json'));
const escape = value =>
  String(value)
    .replaceAll('&', '&amp;')
    .replaceAll('<', '&lt;')
...
const storeAsset = (locale, file) => asset(`${locale === 'en' ? 'en' : 'zh-CN'}/${file}`);
...
for (const image of assetManifest.exports) {
  await mkdir(dirname(`${out}/assets/${image.file}`), { recursive: true });
  await cp(`store/${image.file}`, `${out}/assets/${image.file}`);
}
...
const mediaFiles = {};
for (const image of assetManifest.exports)
  mediaFiles[image.file] = await readFile(`store/${image.file}`);
mediaFiles['sidebrowser.svg'] = await readFile('icons/sidebrowser.svg');
mediaFiles['zh-CN/description.txt'] = new TextEncoder().encode(content.zh.blurb);
mediaFiles['en/description.txt'] = new TextEncoder().encode(content.en.blurb);
mediaFiles['README.txt'] = new TextEncoder().encode(
  'SideBrowser media kit\nChinese store images and copy are in zh-CN/; English images and copy are in en/. Both sets use localized copy and real UI screenshots in the corresponding language. Each folder includes the same store icon; the brand SVG is shared at the root. Generate English or Chinese artwork in light or dark appearance at ' +
    origin +
    href('en', 'media') +
...
function media(locale) {
  ...
  return `<header class="page-heading"><h1>${t.mediaTitle}</h1><p>${t.mediaDescription}</p></header><section class="composer" aria-label="${t.preview}"><div class="canvas-frame"><canvas id="artwork" width="1280" height="800" role="img" aria-label="${t.preview}">${t.preview}</canvas></div><div class="composer-settings"><label for="scene-select">${t.sceneLabel}</label><select id="scene-select">${t.scenes.map((name, i) => `<option value="${i}">${name}</option>`).join('')}</select><button class="button" id="export-image" type="button" disabled>${t.export} <span aria-hidden="true">↓</span></button><p class="fine-print">${t.exportNote}</p><p class="examples-note">${t.examplesNote}</p><p class="status" id="export-status" role="status" aria-live="polite"></p><noscript><p>${locale === 'zh' ? '请启用 JavaScript 生成图片，或下载下方现成素材。' : 'Enable JavaScript to generate images, or download the ready-made assets below.'}</p></noscript></div></section><section><div class="section-heading"><div><h2>${t.library}</h2><p>${t.libraryNote}</p></div><a class="button secondary" href="${asset('downloads/sidebrowser-media.zip')}" download>${t.bundle} ↓</a></div><div class="gallery">${entries.map(({ name, title, size }) => `<figure class="asset"><a class="asset-image" href="${storeAsset(locale, name + '.png')}"><img src="${storeAsset(locale, name + '.png')}" width="${size.split(' × ')[0]}" height="${size.split(' × ')[1]}" alt="${title}" loading="lazy"></a><figcaption class="asset-caption"><div><h3>${title}</h3><p>${size} · PNG</p></div><a class="text-link" href="${storeAsset(locale, name + '.png')}" download>${t.png} ↓</a></figcaption></figure>`).join('')}</div></section><section class="brand-assets"><img src="${asset('sidebrowser.svg')}" width="130" height="130" alt="SideBrowser"><div><h2>${t.logo}</h2><p>${t.logoNote}</p><div class="actions"><a class="text-link" href="${asset('sidebrowser.svg')}" download>${t.svg} ↓</a><a class="text-link" href="${storeAsset(locale, 'store-icon-128.png')}" download>${t.icon} ↓</a></div></div></section><section class="copy-section"><h2><label for="product-copy">${t.copyTitle}</label></h2><textarea id="product-copy" readonly>${escape(t.blurb)}</textarea><div class="actions"><button type="button" class="button secondary" data-copy>${t.copy}</button><span id="copy-status" class="status" role="status" aria-live="polite"></span></div></section>`;
}
const routes = [];

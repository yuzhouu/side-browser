(() => {
  ...
  if (window === window.top || location.ancestorOrigins[0] !== `chrome-extension://${chrome.runtime.id}`) return;
  const detail = JSON.stringify({ extensionId: chrome.runtime.id, identity: globalThis.__pocketMobileIdentity(navigator.userAgent) });
  const reply = () => document.dispatchEvent(new CustomEvent('pocket-mobile-profile-ready', { detail }));

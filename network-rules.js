...
export function panelRules(extensionId, mode, userAgent) {
  ...
  const rules = [
    { id: 1, priority: 1, condition: documents, action: { type: 'modifyHeaders', responseHeaders: [{ header: 'x-frame-options', operation: 'remove' }] } },
    { id: 2, priority: 1, condition: { ...documents, responseHeaders: [{ header: 'content-security-policy', values: ['*frame-ancestors*'] }] },
  ...
  if (mode === 'mobile') {
    const identity = mobileIdentity(userAgent), metadata = identity.userAgentMetadata;
    const brands = list => list.map(item => `"${item.brand}";v="${item.version}"`).join(', ');
    rules.push({ id: 3, priority: 1, condition: requests, action: { type: 'modifyHeaders', requestHeaders: [
      ...
      { header: 'sec-ch-ua-platform-version', operation: 'set', value: JSON.stringify(metadata.platformVersion) },
      { header: 'sec-ch-ua-model', operation: 'set', value: JSON.stringify(metadata.model) },
      { header: 'sec-ch-ua-arch', operation: 'set', value: JSON.stringify(metadata.architecture) },
      { header: 'sec-ch-ua-bitness', operation: 'set', value: JSON.stringify(metadata.bitness) },
      { header: 'sec-ch-ua-full-version', operation: 'set', value: JSON.stringify(metadata.fullVersion) },
      { header: 'sec-ch-ua-full-version-list', operation: 'set', value: brands(metadata.fullVersionList) },

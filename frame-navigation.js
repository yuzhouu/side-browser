(() => {
  if (window === top || location.ancestorOrigins[0] !== `chrome-extension://${chrome.runtime.id}`) return;
  ...
  addEventListener('message', event => {
    ...
    if (event.source !== parent || event.origin !== origin || data?.type !== 'POCKET_VIEWPORT_METRICS'
        || data.documentId !== documentId || !data.mobile) return;
    document.dispatchEvent(new CustomEvent('pocket-mobile-metrics', { detail: JSON.stringify(data) }));
  });
  document.addEventListener('click', event => {
    ...
    const previous = link.getAttribute('target'); link.setAttribute('target', '_self');
    setTimeout(() => { if (previous === null) link.removeAttribute('target'); else link.setAttribute('target', previous); }, 0);
  }, true);
})();

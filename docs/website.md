...
- `store/zh-CN/` 与 `store/en/`：按语言分组的商店文案和图片；`store/assets/source/` 保存真实截图原图。更新原图请遵循 `store/README.md` 的采集流程。
...
商店成品现有简体中文和英文两套：中文使用 `store/zh-CN/`，英文使用 `store/en/`，每套均含 5 张说明图和 2 张宣传图，图标共用。官网的成品素材库、首页分享图和社交预览图随页面语言选择对应图片；下载素材包同时包含两套。英文成品采用独立英文 Chrome 实拍，资料场景为两个 Wikipedia 页面，详见 `../store/en/README.md`。下方生成器仍使用原有三个场景和主题切换规则。

官网与宣传资料的主要定位是「输入任意网址，打开自己常用的网站」。ChatGPT、维基百科与 Google 搜索仅为截图示例，不是网站支持清单；首页、素材选择区和导出图片均说明用户可以自行选择网站。网站自身的登录与嵌入规则仍可能影响使用。

官网 `/media/` 可选 ChatGPT／维基百科／Google 搜索，生成当前语言、当前主题的 1280×800 RGB PNG（无 alpha）。主页面与右侧侧窗放在同一个画面里，并分别标注；主标题、场景解释和三条功能说明位于截图外，截图内容保持真实且不随官网主题重新着色。原图来自中文浏览环境，英文版只翻译说明文字。生成在用户浏览器内完成，不上传数据。下载素材包包含每种语言各 5 张商店说明图、2 张宣传图，以及共用图标、品牌 SVG 和中英文简介。扩展安装 ZIP 独立下载。

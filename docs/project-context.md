...
现有源码有 41 项 Node 测试，包含语言包完整性、占位符、消息引用、错误码翻译，以及最近打开的容量、去重、主动打开入口、跨窗口同步、旧状态保存不能恢复已删除记录，以及 favicon 地址编码与权限、标题获取与变更上报、HTTP 重定向对应条目、内部跳转隔离和删除后的标题清理检查。此前使用独立 Chrome for Testing 149 验证过原生侧边栏、导航、标签切换保活、后台停止与恢复、窗口全屏、模式与请求隔离，以及 NGA 原始公开布局代码的视口比例。逐版本记录见 `verification-history.md`，其中临时截图和脚本路径可能在后续失效。

...
现有源码有 51 项 Node 测试，包含语言包完整性、占位符、消息引用、错误码翻译，以及最近打开的容量、去重、主动打开入口、跨窗口同步、旧状态保存不能恢复已删除记录，以及 favicon 地址编码与权限、标题获取与变更上报、HTTP 重定向对应条目、内部跳转隔离和删除后的标题清理检查。还覆盖通信模块的窗口路由、错误码、端口重连、菜单安装错误和旧脚本缺失时的恢复，以及设置操作的来源校验、模式切换失败后的重试与清空记录不改变窗口导航。外观新增持久化、无效偏好回退、写入失败后重试、侧边栏与设置共用偏好及窗口导航隔离检查；对应浏览器回归场景已补充，本次按用户要求未运行浏览器验证。

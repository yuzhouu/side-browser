...
地址栏右侧时钟按钮提供「最近打开」快捷切换，保留 10 条、重复网址置顶，可一键清空。列表通过独立本地存储键 `pocket-sidepanel-recent-v1` 跨窗口共用并在重启后保留；`recent-urls.js` 负责验证、去重与上限。只有地址输入／搜索、打开当前网页、右键、快捷键和最近条目选择会记录；网页内部跳转、重定向、刷新、模式切换、前进后退、自动恢复与外部新标签打开不记录。不从旧历史推导最近条目，清空不影响当前网页或窗口导航历史，普通 `PANEL_SAVE` 不能写回最近列表。
...
现有源码有 31 项 Node 测试，包含语言包完整性、占位符、消息引用、错误码翻译，以及最近打开的容量、去重、主动打开入口、跨窗口同步、旧状态保存不能恢复已清空记录的检查。此前使用独立 Chrome for Testing 149 验证过原生侧边栏、导航、标签切换保活、后台停止与恢复、窗口全屏、模式与请求隔离，以及 NGA 原始公开布局代码的视口比例。逐版本记录见 `verification-history.md`，其中临时截图和脚本路径可能在后续失效。

...
| 模块 | 负责内容 |
| --- | --- |
| `sidepanel.ts` / `panel-page.ts` | 面板切换、页面实例缓存、独立 iframe 生命周期、导航与操作串行化 |
...
| `panel-client.ts` | 带窗口及目标标签 ID 的请求、错误码还原、消息端口与断线重连 |
| `address-autocomplete.ts` / `address-suggestions.ts` | 地址联想 UI、输入防抖与过期响应隔离、四类来源的只读查询与去重排序 |
...
设置页通过 `SETTINGS_GET`、`SETTINGS_MODE`、`SETTINGS_THEME` 、`SETTINGS_SEARCH_ENGINE` 和 `SETTINGS_CLEAR_RECENT` 请求后台，在同一串行队列中读写模式、外观与最近记录。仅接受扩展自身 `options.html` 的设置请求；侧边栏的导航请求仍要求真实原生 side panel 上下文。设置页监听本地存储变化更新显示，不直接写存储。帮助页不承载设置写入，也不展示开发与验证记录。
...
- 显式标签绑定：移动当前运行网页并清空共享网址／历史及重启恢复状态，两组绑定与多个未绑定共享标签并存，切换保留文档／输入／滚动，单组解绑不影响共享或其他绑定，隐藏页导航隔离、关闭清理、面板刷新及重启后清除绑定（`scripts/browser/tab-pages.mjs`）。
- 停止后台 Service Worker，等待新的 worker 自动启动，现有 iframe 不重建。
- 内部链接、前进后退、刷新、重定向与标题归属。
- 「更多」关闭当前网页后卸载 iframe 并返回欢迎页，清空当前导航、拒绝旧网页排队消息、保留最近记录和其他窗口；关闭加载中网页、从最近列表重开、刷新面板及浏览器重启后的空状态。
- 手机／PC 切换时按设计重载，不新增最近条目。
- 动态标题、最近条目选择、键盘移动、单条删除后的焦点与菜单保持。
- 地址联想：四类来源、同网址去重、键盘／鼠标选择与最近记录、中文输入法、过期查询与 Escape 隔离、点击网页／最近按钮收起，以及原生侧窗与 320px／480px 深浅色模拟布局（`scripts/browser/address-suggestions.mjs`）。
...
搜索引擎偏好由 `search-engine.ts` 定义选项和搜索网址，后台持久化并向各窗口广播；旧导航快照不能覆盖该偏好。`scripts/browser/search-engine.mjs` 检查设置、候选、顶栏关闭和重启恢复；四种搜索网址的浏览器验证用 CDP 提供固定网页响应，不依赖在线搜索服务，也不代表已验证任意地区的真实搜索服务可用性。

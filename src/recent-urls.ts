...
export const normalizeRecentTitle = (value: unknown) =>
  typeof value === 'string' ? value.replace(/\s+/g, ' ').trim().slice(0, 512) : '';

export function restoreRecentTitles(stored: unknown, urls: string[]): Record<string, string> {
  return Object.fromEntries(
    urls.flatMap(url => {
      const title = normalizeRecentTitle(
        stored && typeof stored === 'object' ? (stored as Record<string, unknown>)[url] : undefined
      );
...
export function restoreRecentUrls(stored: unknown): string[] {
  const urls: string[] = [];
...
export function rememberRecentUrl(urls: string[], url: string) {
  return restoreRecentUrls([url, ...urls]);
}

export function recentFaviconUrl(pageUrl: string, extensionRoot: string) {
  ...
  accept(data: FrameMessage) {
    ...
    if (data.type === 'POCKET_LOCATION') {
      if (this.pending) {
        ...
        this.pending = false;
      } else if (data.url !== this.pageUrl || data.documentId !== this.documentId)
        this.entryUrl = data.url;
      this.pageUrl = data.url;
      this.documentId = data.documentId;
    } else if (
      data.type !== 'POCKET_TITLE' ||
      this.pending ||

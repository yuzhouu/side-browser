...
test('recent addresses retain only the last ten unique explicit destinations', () => {
  ...
  assert.deepEqual(
    urls,
    Array.from({ length: 10 }, (_, i) => `https://example.com/${11 - i}`)
  );
...
test('HTTP redirect title and later title changes stay associated with the explicit entry', () => {
  ...
  assert.deepEqual(tracker.accept({ ...page, type: 'POCKET_TITLE', title: 'Loaded title' }), {
    url: entry,
    pageUrl: final,
    title: 'Loaded title'
  });
  assert.deepEqual(tracker.accept({ ...page, type: 'POCKET_TITLE', title: 'New title' }), {
    url: entry,
    pageUrl: final,
    title: 'New title'
  });
...
test('internal navigation titles never overwrite the original entry and old document reports are ignored', () => {
  const tracker = new RecentTitleTracker();
  const entry = 'https://example.com/',
    next = 'https://example.com/inside';
  tracker.navigate(entry);
  tracker.accept({ type: 'POCKET_LOCATION', url: entry, documentId: 'first', title: 'Home' });
  assert.deepEqual(
    tracker.accept({ type: 'POCKET_LOCATION', url: next, documentId: 'first', title: 'Inside' }),
    { url: next, pageUrl: next, title: 'Inside' }
  );
  assert.equal(
    tracker.accept({ type: 'POCKET_TITLE', url: entry, documentId: 'first', title: 'Old URL' }),
    null
  );
  tracker.navigate(entry);
  assert.equal(
    tracker.accept({
      type: 'POCKET_LOCATION',
      url: next,
      documentId: 'first',
      title: 'Old document'
    }),
  ...
  assert.equal(
    tracker.accept({ type: 'POCKET_TITLE', url: next, documentId: 'first', title: 'Old title' }),
    null
  );
  assert.deepEqual(
    tracker.accept({
    ...
    { url: entry, pageUrl: entry, title: 'Home again' }
  );
  assert.equal(
    tracker.accept({ type: 'POCKET_TITLE', url: entry, documentId: 'first', title: 'Outdated' }),
    null
  );
});
